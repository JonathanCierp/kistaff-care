    ...
    <div class="document-upload-item__body">
      <CustomRow class="document-upload-item__header" no-margin>
        ...
        <CustomRow center no-margin>
          <CustomButton
            v-if="userDocuments.length"
            icon-left="IconOutlinedEdit"
            size="sm"
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const userDocuments = computed(() =>
      props.userDocuments?.filter(
        (userDocument) => userDocument.Description === props.label
      )
    );
